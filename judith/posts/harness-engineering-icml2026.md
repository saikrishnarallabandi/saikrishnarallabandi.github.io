...
models converge, orchestration becomes the differentiator" [24]. *Recognize Your
Orchestrator* shows it empirically — across deep-research, coding, GUI, and RAG agents, the
**orchestrator, not the executors, is the primary failure source** [23]. And *Base Models
Know How to Reason, Thinking Models Learn When* argues the capability is already latent; the
control layer decides when it fires [22].
...
harness as systems infrastructure, not a prompt [5]. **OpenSage** goes a step further and
*synthesizes* the scaffold itself: an agent-development kit where the topology, toolset, and
hierarchical memory are generated rather than hand-authored [4]. **MAS-Orchestra** does the
...
benefit is task-dependent, not universal, at >10× efficiency over baselines [3]. The direct
predecessor to all of this is **AFlow** (ICLR 2025 Oral), which framed scaffold generation as MCTS
over code-represented workflows [10].
...
targeted repairs for 6.3–18.4% gains [8]. **The Interplay of Harness Design and Post-Training**
(arXiv 2026 preprint) shows the harness is a variable you must *co-design with training* — neglect
it during training and robustness collapses under environment shift [9].

Rounding out the facet: **DeepHA** builds a hierarchical planner/executor whose Chain-of-Action
cuts context ~75% and hits SOTA on 800+ Minecraft tasks [6]; **AutoTool** learns dynamic
tool selection via a Plackett–Luce ranking that generalizes to unseen tools [1]; **Gecko**
gives the tool-calling inner loop a stateful simulation to refine against, lifting GPT-4o on
BFCLv3 from 76.9%→84.6% [2]; and **Agentic Proposing** composes modular reasoning skills
into verifiable trajectories, reaching 91.6% on AIME25 [7].
...
agents more than ambiguous user requests** [11]. That single result reframes where a harness
...
accuracy, and does it without ground-truth trajectories using small open-source judges [12].
...
must engineer against — retrieval at scale, calibration, and recovery [13]. **AppWorld-UL**
...
function names and arguments, for 3–6× (up to 9.6×) speedups [14]. **D-CORE** incentivizes
task decomposition for complex multi-step tool use [16], and **SciAgentGym** provides a
multi-step scientific tool-use benchmark [17].
...
field has lacked [18]. **SWE-Bench Pro** modernizes code-agent evaluation with a
contamination-resistant public/private split over long-horizon, multi-file tasks [45],
and **daVinci-Dev** (Oral) operationalizes executable-repo environments with real tool outputs as
training substrate, hitting SOTA-open on SWE-Bench Verified at under half the tokens [27].
**SWE-Compass** pushes toward a unified coding-agent evaluation [26].
...
trajectories at ~$0.04 each, no LLM judge required [19]. **CUARewardBench** benchmarks the
*judge itself*, evaluating reward models on computer-using-agent trajectories [20], and
**EnterpriseOps-Gym** offers a containerized, stateful enterprise sandbox where the best agent
manages only 34.1% — and notably fails to *decline infeasible tasks* [21]. **DSGym** extends
the "gym" pattern to data-science agents [25].
...
judge, reproducing 30 attacks with half the code [28]. **SandboxEscapeBench** (Oral) measures
whether an agent can break out of the very container meant to isolate it — a nested-sandbox CTF
built on Inspect-AI — which matters for anyone whose harness executes untrusted agent code [29].
...
"Reasoning Trap" where reasoning-heavy models degrade under context squeeze [23].
...
GAIA/SWE-Bench/Terminal-Bench [30]. **AgentConductor** treats the communication *topology*
as a first-class learnable object, evolving who-talks-to-whom for hard code generation [32].

Two position-shaping results anchor the facet: **AdaptOrch** (preprint) — orchestration is the
differentiator as models converge [24] — and **Base Models Know How to Reason** — control,
not capability, governs realized performance [22]. **InfoPO** adds a concrete harness gate:
a turn-level information-gain reward teaching agents *when to ask versus act* [31].
...
harmful actions in computer-use agents [35]. If harm is a property of the *effect*, then a
...
concrete real-time defense pipeline over agent actions, backed by hierarchical memory [33].
...
Game — an agent gaining autonomy *cannot* decrease the human's value [34]. **ANCHOR** audits
CLI agents (the highest-stakes effect surface) for real-world harmful outcomes [36], and
**TRACER** provides the early-warning signal such a boundary needs, flagging unreliable trajectories
within the first ~20% of a conversation [39].

The prior art here is essential and worth citing precisely: **Progent** (arXiv 2025) is the
canonical programmable-privilege DSL enforced at execution without touching the model [43];
**AgentSpec** (ICSE 2026) is a customizable runtime-enforcement layer intercepting actions at
execution time [42]; and **SafeMCP** (arXiv 2026) filters risky tools from the action space
*before* the agent even chooses — a pre-selection boundary rather than pre-execution [44].
...
causal attribution [37]; **Constitutional Black-Box Monitoring** detects scheming from
behavior alone, no model internals [38]; **AIR** frames safety operationally as
detect→contain→respond [40]; and **Architecture Matters for Multi-Agent Security** shows the
